...
// Staff-only (superadmin/client_admin) — no Owner/Store/Advertiser login sees
// this section. layout.tsx's generic NAV route-guard only matches a path
// EXACT-equal to a NAV href, so a nested route like /prof-performance/admin
// falls through it unguarded (the same gap /calc/marketplace-fee already
// relies on for its own, deliberately looser, read-only-for-Owners access).
// Here that gap would let ANY logged-in role reach these pages, so each page
// re-checks role itself via useStaffOnly() rather than trusting the shell.
...
export function useStaffOnly() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (role !== "superadmin" && role !== "client_admin") { router.replace("/"); return; }
      setReady(true);
    })();
  }, [router]);
  return ready;
