-- =====================================================================
-- 0089: Weekly drilldown for the Dashboard "Monthly Performance" chart.
--
-- dashboard_rollup's grain deliberately excluded `week` (0052/0058), so
-- the finest the chart could show was one bar per month. To let the chart
-- break a selected month into its weeks, `week` is added to the rollup
-- grain + refresh, and dashboard_summary gains a NEW `perf_trend` series
-- that buckets dynamically: by month when p_month IS NULL, by week when a
-- specific month is selected. Note: sales_rows has no true daily date
-- (its `tanggal` is the manually-entered week-start Monday), so WEEK is
-- the finest honest granularity here — not day.
--
-- perf_trend is additive: every existing series (monthly_sales,
-- traffic_trend, cost_roas, sparklines, etc.) is untouched and stays
-- monthly. Only the one "Monthly Performance" chart reads perf_trend.
-- Adding `week` to the grain is backward-compatible: existing
-- month-grouped CTEs simply collapse the extra rows, and the KPI SUMs are
-- unaffected.
-- =====================================================================

alter table dashboard_rollup add column if not exists week text;
create index if not exists dashboard_rollup_month_week_idx on dashboard_rollup (client_id, month, week);
...
begin
  ...
  insert into dashboard_rollup (
    client_id, year, month, week, city, store_name, brand, product_type, item_name, source, ad_type,
  ...
  group by client_id, year, month, week, city, store_name, brand, product_type, item_name, source, ad_type;
...
as $$
  ...
  select jsonb_build_object(
    ...
    -- Feeds ONLY the "Monthly/Weekly Performance" chart.
