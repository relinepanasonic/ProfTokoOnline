-- =====================================================================
-- 0026: Per-store monthly SPOS sales trend for the "Detail Data per Store"
-- sparkline. Ignores the p_month filter (a sparkline needs the whole
-- timeline, not a single month) but respects year/city/owner/brand/store.
-- =====================================================================
  ...
  select jsonb_build_object(
    ...
    'dealers', (select coalesce(jsonb_agg(x order by x.sales desc nulls last),'[]') from (
        select
          ...
          (select coalesce(jsonb_agg(t order by array_position(
                     array['Januari','Februari','Maret','April','Mei','Juni','Juli',
                           'Agustus','September','Oktober','November','Desember'], t.month)),'[]')
           from (
             select month, sum(sales_idr) sales
             from f_trend ft
             where ft.store_name = d.store_name and ft.source = 'spos'
             group by month
           ) t) as trend
        from f_real d
        where d.store_name is not null
        group by d.store_name, d.city) x)
  );
$$;
