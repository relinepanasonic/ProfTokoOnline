-- =====================================================================
-- 0046: Kill the ~950MB temp-disk spill in dashboard_summary().
--
-- ROOT CAUSE (found via EXPLAIN ANALYZE — "temp read=121093" blocks =
-- ~950MB spilled to disk, ~10s of pure I/O):
--   The base/f_real CTEs did `select s.*`, which carries the huge `raw`
--   jsonb column (~3KB/row × 108k rows ≈ 326MB) through CTE
--   materialization. Both CTEs are referenced many times so Postgres
--   materialises them, writing every bloated `raw` blob to temp storage.
--   The aggregations never touch `raw` — only ~19 small numeric/text cols.
--
-- FIX: enumerate exactly the columns the function uses in the base CTE,
--   so `raw` (and other unused wide columns) never enter materialization.
--   Materialised row shrinks from ~3KB to ~100 bytes -> no spill ->
--   query drops from ~10s to sub-second, comfortably under the role
--   statement_timeout even on the authenticated (RLS) path.
--
-- Nothing about WHAT is computed changes — identical body to 0044 except
-- the base CTE's column list.
-- =====================================================================
...
as $$
  with base as (
    select s.year, s.city, s.store_name, s.source, s.is_parent, s.month,
           s.sales_idr, s.visitors, s.in_cart, s.orders, s.orders_ready,
