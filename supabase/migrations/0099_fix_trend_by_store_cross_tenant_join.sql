-- =====================================================================
-- 0099: Fix cross-tenant join explosion in dashboard_summary's
-- trend_by_store CTE (57014 for Super Admin / client_admin).
--
-- trend_by_store joins dashboard_month_completeness on store_name+month
-- ONLY — no client_id. `base` (and therefore `b`) is already scoped to
-- one tenant via `r.client_id = p_client_id`, but `c` is not: under RLS,
-- superadmin's completeness_admin_read policy grants unconditional
-- access to EVERY tenant's completeness rows (same unscoped shape as
-- rollup_admin_read, by design — see 0056). So a store/month in one
-- tenant can match a same-named store/month completeness row belonging
-- to a DIFFERENT tenant, fanning out the join.
--
-- This join shape has existed since migration 0058 — it is not something
-- 0097/0098 introduced. It's surfacing now because the platform has
-- accumulated many self-registered trial tenants (/register), each with
-- user-typed store names, increasing the odds of a same-name collision
-- across tenants for exactly the kind of generic name a signup form
-- produces.
--
-- Fix: bind the join to p_client_id explicitly, so it can never match
-- another tenant's rows regardless of what RLS permits. One-line change;
-- everything else in the function is identical to 0098.
-- =====================================================================
